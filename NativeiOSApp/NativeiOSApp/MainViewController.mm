...
#include "SwiftFuntionMediator.h"
...
- (void)viewDidLoad
{
    ...
    self.bgImageView.image =[UIImage imageNamed:@"splash"];
...
//    // INIT UNITY
    ...
    [self.unityInitBtn setTitle: @"" forState: UIControlStateNormal];
    [self.unityInitBtn setHidden: YES];
    [self.unityInitBtn addTarget: hostDelegate action: @selector(initUnity) forControlEvents: UIControlEventTouchUpInside];
    [self.view addSubview: self.unityInitBtn];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveNotificationFromMobFlowSDK:) name:@"receiveNotificationFromMobFlowSDK" object:nil];
}

- (void) receiveNotificationFromMobFlowSDK:(NSNotification *) notification
{
    ...
    //if ([SwiftFuntionMediator showUnityAds]) {
        NSLog(@"Show Portal button");
        
        showPortalButton = [SwiftFuntionMediator showUnityAds];
        
//        UIStoryboard *paymentStoryboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
//        UINavigationController *edView = [[UINavigationController alloc] init];
//        edView = [paymentStoryboard instantiateViewControllerWithIdentifier:@"idPremiumHome"];
//        AppContext.window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
//        [AppContext.window setRootViewController:edView];
//        [AppContext.window makeKeyAndVisible];
        
        [self.unityInitBtn sendActionsForControlEvents:UIControlEventTouchUpInside];
//    } else {
//        NSLog(@"Show Portal button : False");
//        showPortalButton = NO;
//
//        UIStoryboard *paymentStoryboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
//        UINavigationController *edView = [[UINavigationController alloc] init];
...
@property (nonatomic, strong) UIButton *quitBtn;
@property (nonatomic, strong) UIImageView *bgImageView;
@property (weak, nonatomic) IBOutlet UIButton *premiumPortalBtn;
@property (weak, nonatomic) IBOutlet UILabel *premiumSname;
@property (weak, nonatomic) IBOutlet UIButton *premiumUnityGameBtn;
...
//    // INIT UNITY
    ...
    self.premiumSname.text = [SwiftFuntionMediator fetchSname];
//    // SHOW UNITY
...
//    if (showPortalButton) {
//        NSLog(@"Home page added");
//        UIView *homeafterbilling = [[[NSBundle mainBundle] loadNibNamed:@"HomeAfterBilling" owner:self options:nil] firstObject];
//        [self.view addSubview:homeafterbilling];
//    }
    
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveTestNotification:) name:@"TestNotification" object:nil];
    
    if (![SwiftFuntionMediator showUnityAds]) {
        [self.premiumPortalBtn setHidden:YES];
        [self.premiumSname setHidden:YES];
        [self.premiumUnityGameBtn setHidden:YES];
    }
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (![SwiftFuntionMediator showUnityAds]) {
        [self.premiumPortalBtn setHidden:YES];
        [self.premiumSname setHidden:YES];
        [self.premiumUnityGameBtn setHidden:NO];
    }
}

-(IBAction)btnPortalButtonAction:(UIButton *)sender {
    [SwiftFuntionMediator showPremiumPortal];
}

-(IBAction)btnLoadUnityGame:(id)sender {
    [self.unityInitBtn sendActionsForControlEvents:UIControlEventTouchUpInside];
}
...
- (void)CallUnityIntialiseSet:(NSString*)isSet
{
    
    // you will receive true here
    NSLog(@"CallUnityIntialiseSet Called");
    
    [[self ufw] sendMessageToGOWithName: "UnityAdsManager" functionName: "SetAdvertisement" message: (showPortalButton ? "true" : "false")];
    [[self ufw] sendMessageToGOWithName: "UnityAdsManager" functionName: "SetBackButtonFlag" message: ("false")];
    

//    [[self ufw] sendMessageToGOWithName: "UnityAdManager" functionName: "SetBackButtonFlag" message: "true"];
    
}

- (void)CallAdsStatusFromiOS:(NSString*)isShow
{
    // reference to set ads flag
    NSLog(@"CallAdsStatusFromiOS Called");
    [[self ufw] sendMessageToGOWithName: "UnityAdsManager" functionName: "SetAdvertisement" message: "true"];
}

- (void)CallBackButtonStatusFromiOS:(NSString*)isShow
{
    //reference to set back button flag
    NSLog(@"SetBackButtonFlag Called");
    [[self ufw] sendMessageToGOWithName: "UnityAdsManager" functionName: "SetBackButtonFlag" message: "true"];
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    //ViewController *viewcontrol ler = [[ViewController alloc] initWithNibName:nil Bundle:nil];
    ...
    [SwiftFuntionMediator callinitMobflow];
    return YES;
}
